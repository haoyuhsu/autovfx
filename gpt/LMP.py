...
from .gpt4v_utils import encode_image
...
class LMP:
    """Language Model Program (LMP), adopted from Code as Policies & VoxPoser."""
    ...
    def _cached_api_call(self, **kwargs):
        ...
        if kwargs['model'] != 'gpt-3.5-turbo-instruct' and \
            ...
            user1 = f"I would like you to help me write Python code to perform editing on a 3D scene. Please complete the code every time when I give you new query. Pay attention to appeared patterns in the given context code. Be thorough and thoughtful in your code. Do not include any import statement. Do not repeat my question. Do not provide any text explanation (comment in code is okay). I will first give you the context of the code below:\n\n```\n{user1}\n```\n\n"
            ...
            messages=[
                {"role": "system", "content": "You are a helpful assistant that pays attention to the user's instructions and writes good python code for performing editing on a 3D scene."},
...
def exec_safe(code_str, gvars=None, lvars=None):
    ...
    try:
        ...
        code_str = f'from utils.general_utils import safe_state' + '\n' + \
                f'from scene_representation import SceneRepresentation' + '\n' + \
                f'from edit_utils import *' + '\n' + \
                f'from gpt.code_gen import setup_LMP' + '\n' + \
                f'from opt import get_opts' + '\n' + \
                f'import numpy as np' + '\n' + \
                f'hparams = get_opts()' + '\n' + \
                f'#safe_state(hparams.quiet)' + '\n' + \
                f'scene = SceneRepresentation(hparams)' + '\n' + \
                f'scene.render_from_3DGS()' + '\n' + \
                code_str + '\n' + \
                f'scene.render_scene(skip_render_3DGS=False)' + '\n'
