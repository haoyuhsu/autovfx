...
import ram_utils
from ram_utils import warmup_lr_schedule, step_lr_schedule
...
def train_ram_plus(model, data_loader, optimizer, epoch, device, config, model_clip):
    ...
    metric_logger = ram_utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', ram_utils.SmoothedValue(window_size=50, fmt='{value:.6f}'))
    metric_logger.add_meter('loss_tag', ram_utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    metric_logger.add_meter('loss_dis', ram_utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    metric_logger.add_meter('loss_alignment', ram_utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
...
def train_ram(model, data_loader, optimizer, epoch, device, config, model_clip):
    ...
    metric_logger = ram_utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', ram_utils.SmoothedValue(window_size=50, fmt='{value:.6f}'))
    metric_logger.add_meter('loss_t2t', ram_utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    metric_logger.add_meter('loss_tag', ram_utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
    metric_logger.add_meter('loss_dis', ram_utils.SmoothedValue(window_size=50, fmt='{value:.4f}'))
...
def main(args, config):
    ram_utils.init_distributed_mode(args)    
    
    device = torch.device(args.device)

    # fix the seed for reproducibility
    seed = args.seed + ram_utils.get_rank()
    ...
    num_tasks = ram_utils.get_world_size()
    global_rank = ram_utils.get_rank()            
