...
def save_result(queue: Queue):
    while True:
        ...
        if save_the_mask:
            ...
            ###############################################################
            # custom saving for demo dataset (save each instance separately)
            if saver.dataset == 'demo':
                for id in all_obj_ids:
                    out_path = path.join(saver.output_root, str(id))
                    os.makedirs(out_path, exist_ok=True)
                    instance_mask = np.array(out_mask == id)  # turn obj_mask into a binary mask
                    result_mask = np.stack([instance_mask * 255] * 3, axis=-1).astype(np.uint8)
                    result_mask = Image.fromarray(result_mask)
                    result_mask.save(path.join(out_path, frame_name[:-4] + '.png'))
            ###############################################################
