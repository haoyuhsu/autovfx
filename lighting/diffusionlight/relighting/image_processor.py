...
def pil_center_crop_and_resize(image, desired_size=(512, 512), interpolation=Image.LANCZOS):
    """
    Center crop and resize an image to a desired size.
    """
    # Don't resize if already the desired size
    if image.size == desired_size:
        return image

    # Calculate aspect ratios
    img_aspect = image.width / image.height
    desired_aspect = desired_size[0] / desired_size[1]

    if img_aspect > desired_aspect:
        # The image is wider than the desired aspect ratio, crop width
        new_width = int(desired_aspect * image.height)
        new_height = image.height
    else:
        # The image is taller than the desired aspect ratio, crop height
        new_width = image.width
        new_height = int(image.width / desired_aspect)
    ...
    image_resized = image_cropped.resize(desired_size, interpolation)
