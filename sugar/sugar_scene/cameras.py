...
def load_gs_cameras(source_path, gs_output_path, image_resolution=1, 
                    load_gt_images=True, load_omni_depth=False, load_omni_normal=False, max_img_size=1920):  # original max_img_size=1920
    ...
    for cam_idx in range(len(camera_transforms)):
        ...
        normal_path = os.path.join(normal_dir,  name + '_normal.png')
        ...
        if load_gt_images:
            ...
            depth, normal = None, None
            # depth = np.load(depth_path) if os.path.exists(depth_path) and load_omni_depth else None
            # normal = np.load(normal_path) if os.path.exists(normal_path) and load_omni_normal else None
            normal = np.array(Image.open(normal_path)) if os.path.exists(normal_path) and load_omni_normal else None
