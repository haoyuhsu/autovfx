...
from gaussian_splatting.utils.loss_utils import depth_loss, opacity_loss, normal_loss, sparsity_loss, anisotropic_loss
...
def coarse_training_with_density_regularization(args):
    ...
    num_iterations = int(args.iterations)  # Changed
    ...
    lambda_pseudo_normal = float(args.lambda_pseudo_normal)
    lambda_alpha = float(args.lambda_alpha)
    lambda_anisotropic = float(args.lambda_anisotropic)
    ...
    for batch in range(9_999_999):
        ...
        for i in range(0, train_num_images, train_num_images_per_batch):
            ...
            if not no_rendering:
                ...
                if lambda_depth > 0 and nerfmodel.cam_list[camera_indices].depth is not None:
                ...
                if lambda_normal > 0 and nerfmodel.cam_list[camera_indices].normal is not None:
                ...
                if lambda_alpha > 0:
                    loss_alpha = opacity_loss(sugar.strengths.view(-1, 1))    # this makes background overly smooth
                    loss_alpha = sparsity_loss(sugar.strengths.view(-1, 1))   # this makes background overly smooth
                    loss = loss + lambda_alpha * loss_alpha

                # Anisotropic Loss
                loss_anisotropic = torch.tensor(0.0, device="cuda")
                if lambda_anisotropic > 0:
                    loss_anisotropic = anisotropic_loss(sugar.scaling)
                    loss = loss + lambda_anisotropic * loss_anisotropic
                #############################################################
