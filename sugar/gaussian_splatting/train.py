...
from utils.loss_utils import l1_loss, ssim, depth_loss, opacity_loss, normal_loss, sparsity_loss, anisotropic_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, use_wandb = False, max_img_size = 1920):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if opt.lambda_depth > 0 and viewpoint_cam.depth is not None:
        ...
        if opt.lambda_normal > 0 and viewpoint_cam.normal is not None:
        ...
        if opt.lambda_alpha > 0:
            loss_alpha = opacity_loss(gaussians.get_opacity)    # this makes background overly smooth
            loss_alpha = sparsity_loss(gaussians.get_opacity)   # this makes background overly smooth
